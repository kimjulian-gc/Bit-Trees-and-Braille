...
public class BrailleASCII {
  public static void main(String[] args) throws IOException {
    if (args.length != 2) {
      throw new IllegalArgumentException("Invalid number of arguments: got " + args.length + ", needs 2");
    }
  ...
  public static String convertToASCII(String bits) throws IOException {
    ...
    if (!isString) return BrailleASCIITables.toASCII(bits);

    // bits is a string of braille
    String output = "";
    for (int i = 0; i < bits.length(); i += 6) {
      String subBits;
      if (i + 6 <= bits.length()) subBits = bits.substring(i, i + 6);
      else subBits = bits.substring(i);
